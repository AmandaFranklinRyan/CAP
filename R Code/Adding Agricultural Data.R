...
mydb <- dbConnect(SQLite(),"SQL Database//Geographic Enterprise Data 4.sqlite3")

#Read csv into database in chunks
read_csv_chunked("Cleaned Data//Geographic Data Enterprises Final.csv", 
                 callback = function(chunk, dummy){
                   dbWriteTable(mydb, "Geographic Enterprise Data 4", chunk, append = T)}, 
                 chunk_size = 10000) #col_types = "cccccclclllllcclcl")

agriculturedb <- tbl(mydb, "Geographic Enterprise Data 4")
